...
import { isReservedSlug, generateUniqueSlugFromBase } from '@/lib/admin';
import { extractFirstNameToken, isAutoGeneratedSlug } from '@/lib/slug-format';
...
export async function startWeddingQuickStart(
  ...
  try {
    ...
    // ─── 5b. Slug bonito en la PRIMERA completada del wizard ───────────────
    // boda-<novia>-y-<novio> (sin sufijo si está libre). Solo si:
    //   1. es la primera completada (modo initial y flag aún no marcado),
    //   2. la invitación no está publicada, y
    //   3. el slug actual es autogenerado — nunca uno personalizado a mano.
    // El link no viaja en ningún email post-compra, así que renombrar aquí es
    // seguro; después de esta ventana el slug ya no se toca.
    let finalSlug = current.slug;
    const wizardAlreadyCompleted =
      (current.hero as { wizardExpressCompleted?: boolean } | undefined)?.wizardExpressCompleted === true;
    const isFirstCompletion = mode === 'initial' && !wizardAlreadyCompleted;
    const isPublished = current.status === 'published' || !!current.publishedAt;
    if (isFirstCompletion && !isPublished && isAutoGeneratedSlug(current.slug)) {
      const brideToken = extractFirstNameToken(brideName);
      const groomToken = extractFirstNameToken(groomName);
      if (brideToken && groomToken) {
        try {
          finalSlug = await generateUniqueSlugFromBase(`boda-${brideToken}-y-${groomToken}`);
          console.log('[QuickStart] Slug renombrado: %s → %s', current.slug, finalSlug);
        } catch (slugErr) {
          finalSlug = current.slug;
          console.warn('[QuickStart] Renombre de slug falló (no fatal):', slugErr);
        }
      }
    }
    ...
    if (finalSlug !== current.slug) {
      revalidatePath(`/i/${finalSlug}`);
      revalidatePath(`/${finalSlug}`);
      revalidatePath(`/invitaciones/${finalSlug}`);
    }
